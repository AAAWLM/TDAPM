...
def train_target(args):
    ...
    for i in range(len(args.src)):
        ...
        for parameter in netC_list[i].parameters():
            # C is the fixed source classifier; gradients still flow to B/F.
    ...
    acc_init = 0
    iter_num_update = 0
    # Create the shuffled target iterator before the first optimizer step.
    # Subsequent epochs are rebuilt only after a normal StopIteration.
    ...
    while iter_num < max_iter:
        ...
        # Refresh at the released-code cadence.  It must also run at iter=0
        # so pseudo-label/centroid state exists before the first loss.
        if iter_num % C == 0:
            iter_num_update += 1
            initc = []
            all_feas = []
        ...
        if start_test:
            ...
            # torch.cat 连接
            # 将当前小批量的输出张量与之前的 all_output 沿着指定维度（0 维，即行）连接起来
            all_output = torch.cat((all_output, outputs_all_w.float().cpu()), 0)
            # 将当前小批量的瓶颈层特征张量与之前的 all_feature_F 沿着指定维度连接起来。
            all_feature = torch.cat((all_feature, features_all_w.float().cpu()), 0)
            # 将当前小批量的特征张量与之前的 all_feature 沿着指定维度连接起来
        ...
        # if start_in:
        #     all_output_N = outputs_all_w_N.float().cpu()
        #     start_in = False
        # else:
        #     all_output_N = torch.cat((all_output_N, outputs_all_w_N.float().cpu()), 0)
